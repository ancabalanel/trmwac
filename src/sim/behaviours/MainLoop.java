 ...
 * <code>Sensor</code> agent, the agent continuously handles instructions
 * and broadcast frames coming from the <code>Simulation</code> agent.
 * 
 * @author Anca
 * @see Instruction
 * @see Frame
...
public class MainLoop extends CyclicBehaviour{
	...
	static MessageTemplate templateReceiveInstruction = MessageTemplate.MatchPerformative(ACLMessage.REQUEST);
	/** Template for receiving broadcasted frames (dispatched by the Sim agent) */
	static MessageTemplate templateReceiveFrame = MessageTemplate.MatchPerformative(ACLMessage.PROPAGATE);
